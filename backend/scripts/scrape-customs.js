#!/usr/bin/env node
// Scrape 關務特考三等 MCQ subjects using pdfjs-dist position data
// Subjects: 法學知識 (50Q, 114 only) + 英文 (25Q, 110-114)
...
async function main() {
  ...
  for (const sess of SESSIONS) {
    ...
    for (const sub of SUBJECTS) {
      ...
      const isMixed = sub.mixedEssay || (sub.mixedEssayYears && sub.mixedEssayYears.includes(sess.year))
      if (isMixed) {
