#!/usr/bin/env node
/**
 * One-shot scraper for 中醫師(一) 106-109 共 4 場次（共 8 PDF, ~640 題）。
 *
 * 106/107/109 → 030 系列, 108 → 020 系列. All use c=101, s=0101/0102.
 * PDF format: same as 110-111 (no ABCD labels, column-positioned options).
 * Reuses mupdf column-aware parser from scrape-tcm1-110-111.js.
 *
 * Usage:
 *   node scripts/scrape-tcm1-106-109.js
 *   node scripts/scrape-tcm1-106-109.js --dry-run
 */
...
const SESSIONS = [
  { year: '106', code: '106030', label: '第一次', classCode: '101' },
  { year: '106', code: '106110', label: '第二次', classCode: '101' },
  { year: '107', code: '107030', label: '第一次', classCode: '101' },
  { year: '107', code: '107110', label: '第二次', classCode: '101' },
  { year: '108', code: '108020', label: '第一次', classCode: '101' },
  { year: '108', code: '108110', label: '第二次', classCode: '101' },
  { year: '109', code: '109030', label: '第一次', classCode: '101' },
  { year: '109', code: '109110', label: '第二次', classCode: '101' },
]
...
// Fallback: pdf-parse based answer parser for older format
async function parseAnswersPdfParse(buf) {
  const { text } = await pdfParse(buf)
  const answers = {}
  // Fullwidth format: 答案ＡＢＣＤ...
  ...
  for (const sess of SESSIONS) {
    ...
    for (const sub of SUBJECTS) {
      ...
      try {
        ...
        // Try mupdf answer parser first, fallback to pdf-parse
        let answers = {}
        if (sBuf) {
          answers = await parseAnswersColumnAware(sBuf)
          if (Object.keys(answers).length < 5) {
            answers = await parseAnswersPdfParse(sBuf)
          }
        }
