...
const{data:rps}=await sb.from('reports').select('*').gt('created_at',process.argv[2]||'2026-07-19T17:00:00Z').order('created_at',{ascending:true})
const{data:fbs}=await sb.from('feedback').select('*').gt('created_at',process.argv[2]||'2026-07-19T17:00:00Z').order('created_at',{ascending:true})
