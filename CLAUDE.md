...
- **一律用繁體中文回覆**，包括說明、建議、問題回報、程式碼注釋以外的所有文字
...
遇到指令執行失敗、結果異常、或發現前提假設有誤時：
- **立即停下**，不要繼續跑後續步驟
- **先回報用戶**：「發現問題：XXX，建議改法：YYY，是否繼續？」
- 等用戶確認後才繼續

**反面範例**：fill-gaps 腳本第二次執行時 143/144 個目標都是 302 失敗，但腳本繼續跑完才回報。應在首批連續失敗時立即中斷並回報 URL 可能不對。
...
### 10. HTTP 302 不代表 PDF 已被考選部下架
- **錯誤假設**：HTTP 302 → PDF 永久刪除，無法修復
- **事實**：考選部不會下架題目。302 表示參數錯誤（場次代碼、class code、subject code）
- **案例**：doctor2 110080 全部回 302，原因是用 c=301 而非 c=302；113020 醫學六 用 c=302 s=44 即回 HTTP 200
- **如何避免**：
  1. 先到考選部試題查詢頁面確認考試代碼、類科代碼、科目代碼
  2. 用 dry-run 或 test script 逐個測試參數組合
  3. 找到 HTTP 200 後再確認 PDF 內容（驗考試名稱）
  4. 不要假設 302 表示永久無法修復
...
**030 系列（高等考試合併場）class code：**

| 年度 | 場次 | c=101 | c=102 | c=103 | c=104 | c=105 | c=106 | c=107 | c=109 | c=110 |
|------|------|-------|-------|-------|-------|-------|-------|-------|-------|-------|
| 100 | 030 | 醫師 | — | 藥師 | 醫檢師 | 護理師 | — | 中醫師 | — | — |
| 100 | 140 | — | — | 藥師 | — | 護理師 | 中醫師 | 營養師 | — | — |
| 101 | 030 | 醫師 | 醫師二階 | 藥師 | 醫檢師 | 護理師 | 中醫師 | 營養師 | — | — |
| 102 | 030 | 醫師 | 醫師二階 | 中醫師合 | 中醫師(一) | 中醫師(二) | 營養師 | 臨床心理師 | 醫檢師 | 護理師 |
| 103 | 030 | 醫師 | 醫師二階 | 中醫師(一) | 中醫師(二) | 中醫師合 | 營養師 | 臨床心理師 | 護理師 | 社工師 |
| 103 | 100 | 醫師 | 醫師 | 中醫師(一) | 中醫師(二) | 中醫師合 | 營養師 | 臨床心理師 | 護理師 | 社工師 |
| 104 | 030 | 醫師 | 醫師二階 | 中醫師(一) | 中醫師(二) | 中醫師合 | 營養師 | — | 護理師 | 社工師 |
| 104 | 100 | 中醫師(一) | 中醫師(二) | 營養師 | — | — | 護理師 | 社工師 | — | — |
| 105 | 030 | 中醫師(一) | 中醫師(二) | 營養師 | — | — | 護理師 | 社工師 | — | — |
| 105 | 090 | 中醫師(一) | 中醫師(二) | 營養師 | — | — | 護理師 | 社工師 | — | — |

⚠️ **102/103年 030系列 class code（2026-05-17 PDF probe 修正）**：原表記載 c=103/104=中醫師、c=107=護理師有誤，導致 102030/103030/103100 的護理師（誤抓臨床心理師）、中醫師一二（誤抓醫檢師/護理師/社工師）共 25 卷約 2,000 題污染。正確：tcm1 第一試→102030 c=104、103030/103100 c=103（s=0201,0202）；tcm2 第二試→102030 c=105、103030/103100 c=104（s=0203-0206）；護理師→102030 c=110（s=0108,0601-0604）、103030/103100 c=109（s=0107,0501-0504）。已重抓並補滿（驗證工具 `scripts/verify-100-105-papers.js`）。
⚠️ **100年 140 場次（第二次）030系列**：c=101=醫師一, c=102=醫師二, c=103=藥師一+二, c=104=醫檢師, c=105=護理師, c=106=中醫師(一+二), c=107=營養師。中醫師(一)/(二) 共用 c=106（s=0501-0502=tcm1, s=0503-0506=tcm2）。中醫師(一) 中醫基礎醫學(二) s=0502 為全圖像 PDF，已用 OCR Vision 補齊（2026-04-19）。
⚠️ **101年 110 場次（第二次）030系列**：c=101=醫師一(s=0101-0102), c=102=醫師二(s=0103-0106), c=103=中醫師合併(s=0201-0202 tcm1 + s=0203-0206 tcm2), c=104=中醫師第一試(s=0201-0202 tcm1 only, 與 c=103 重複故跳過), c=105=營養師(s=0301-0306), c=107=心理師(跳過), c=108=醫檢師(s=0107,0501-0505), c=109=護理師(s=0108,0601-0604), c=110=法醫師(跳過)。2026-04-20 爬回 3,029 題。
⚠️ **101年 100 場次（第二次）020系列**：c=307=獸醫師(s=11-66), c=310=藥師合併(s=11,22,44=pharma1 + s=33,55,66=pharma2)。2026-04-20 補回 930 題（vet 480 + pharma 450）。
⚠️ **100年 130 場次（第二次）020系列**：c=301=牙醫(一), c=302=牙醫(二) 4 papers (s=33-66), c=305=職治, c=307=獸醫, c=308=放射, c=309=物治。c=306=呼吸治療師（不在平台內跳過）。2026-04-19 爬回 3,792 題。
⚠️ **Nutrition 102-103年：** 不在 030 series 數位化範圍（MoEX 查無 PDF）。
...
**105年 020 系列 class code 大換位（2026-04-18 確認）：**

| c code | 105年指向考試 | 備注 |
|--------|------------|------|
| 305 | 藥師一階 | 改自原 OT (101-104) |
| 307 | 藥師二階 | s=44,55,66 = 調劑/藥治/法規 |
| 308 | 醫事檢驗師 | 改自原 放射師 (100-104) |
| 309 | 醫事放射師 | 改自原 物治師 (101-104) |
| 311 | 物理治療師 | 改自原 醫檢師 (102-104) |
| 312 | 職能治療師 | 改自原 藥師一階 (103-104) |
...
|------|---------|---------|------|
| 100 | 100030 | 100020 | — |
| 101 | 101030 (一次), 101110 (二次) | 101010 (一次), 101100 (二次) | — |
...
| 考試 | 題數 | 年度範圍 | 不完整題 |
|------|------|---------|---------|
| 物治師 | 13,919 | 100-115 | 113 |
| 放射師 | 13,905 | 100-115 | 266 |
| 醫檢師 | 12,951 | 100-115 | 675 |
| 職治師 | 10,966 | 100-114 | 133 |
| 中醫師(二) | 9,249 | 100-115 | 56 |
| 護理師 | 9,810 | 100-115 | 543 |
| 醫師二階 | 8,936 | 100-115 | 184 |
| 牙醫師(二) | 7,920 | 100-115 | 116 |
| 藥師一階 | 6,897 | 100-115 | 201 |
| 營養師 | 6,146 | 100,101,104-115 | 245 |
| 藥師二階 | 5,619 | 100-115 | 313 |
| 醫師一階 | 6,176 | 100-115 | 44 |
| 中醫師(一) | 4,624 | 100-115 | 48 |
| 牙醫師(一) | 4,319 | 100-115 | 116 |
| 社工師 | 2,622 | 104-115 | 63 |
| 司法特考三等 | 444 | 106-114 | — |

注意：
- 營養師 100/102/103 年題目已移除（原為錯誤的中醫師 class code 資料）；100年第二次（100140 c=107）已補回 240 題
- 社工師 112-114 第二次（112110, 113100, 114100, c=103）已於 2026-04-17 新增
- 職治師/獸醫師 110年起只有第二次（第一次考試停辦），非資料缺口
- 醫師一階 102年第二次（102110 c=101 s=0101,0102）已補回 199 題（2026-04-21）
- 醫師一階 104年第二次（104090 c=301 s=55,66）已補回 198 題（修正 parseQuestions 80 題上限 → 200）
- 醫師一階 105年第一/二次（105020 / 105100 c=301）已補回 297 題（2026-04-21，CBT 年 PDF 實際可下載）
- 醫師一階 106年第一次 醫學(二)（106020 c=301 s=66）已補回 99 題（2026-04-21）
- 醫師一階 106-107年第一次：部分仍缺（106020 c=301 s=55 / 107020 需個別確認）
- 護理師/中醫師(一)/(二)/營養師 100年第二次（100140）已補回；中醫師(一) 中醫基礎醫學(二) 為全圖像 PDF，0/80 題解析（嵌圖）
...
| 中醫師(一) | 104 | 第一次 | `https://wwwq.moex.gov.tw/exam/wHandExamQandA_File.ashx?t=Q&code=104030&c=103&s=0201&q=1` |
| 中醫師(二) | 104 | 第一次 | `https://wwwq.moex.gov.tw/exam/wHandExamQandA_File.ashx?t=Q&code=104030&c=104&s=0203&q=1` |
| 護理師 | 104 | 第一次 | `https://wwwq.moex.gov.tw/exam/wHandExamQandA_File.ashx?t=Q&code=104030&c=109&s=0501&q=1` |
| 營養師 | 104 | 第一次 | `https://wwwq.moex.gov.tw/exam/wHandExamQandA_File.ashx?t=Q&code=104030&c=106&s=0301&q=1` |
| 社工師 | 104 | 第一次 | `https://wwwq.moex.gov.tw/exam/wHandExamQandA_File.ashx?t=Q&code=104030&c=110&s=0601&q=1` |
| 醫師(一) | 104 | 第二次 | `https://wwwq.moex.gov.tw/exam/wHandExamQandA_File.ashx?t=Q&code=104090&c=301&s=55&q=1` |
| 護理師 | 100 | 第二次 | `https://wwwq.moex.gov.tw/exam/wHandExamQandA_File.ashx?t=Q&code=100140&c=105&s=0108&q=1` |
| 中醫師 | 100 | 第二次 | `https://wwwq.moex.gov.tw/exam/wHandExamQandA_File.ashx?t=Q&code=100140&c=106&s=0501&q=1` |
| 營養師 | 100 | 第二次 | `https://wwwq.moex.gov.tw/exam/wHandExamQandA_File.ashx?t=Q&code=100140&c=107&s=0601&q=1` |
...
| 項目 | 修復日期 | 說明 |
|------|---------|------|
| 護理師/中醫師 102-103 跨考試污染（25 卷約 2,000 題） | 2026-05-17 | scrape-100-105.js 的 102/103 場次 class code 指錯（護理師→臨床心理師、中醫師→醫檢/護理/社工）。逐卷驗 757 卷 PDF 類科揪出 25 卷，PDF probe 重建正確代碼，刪汙重抓 + Vision OCR 補 139 題，7 卷全 100% 完整。新工具：verify-100-105-papers.js / probe-tcm-nursing-102-103.js / audit-paper-contamination.js |
...
| tcm2 102030 臨床(一)-(四) + tcm1 102/103 零星污染 | 2026-04-20 | 臨床卷從 c=103 s=0203-0206 完整重爬（+319 題），tcm1 7 題目標替換 |
| 101年第二次 101110+101100 全線補齊 | 2026-04-20 | +3,029 題 across doctor1(198)/doctor2(319)/tcm1(160)/tcm2(318)/nutrition(240)/medlab(467)/nursing(397)/vet(480)/pharma1(240)/pharma2(210)。發現 101110 為 030 系列第二次全新場次代碼 |
| 100年第二次 100140+100130 全線補齊 | 2026-04-19 | +3,792 題 across doctor1/doctor2/pharma1/pharma2/medlab/dental1/dental2/ot/radiology/pt/vet（vet 首次加入 scrape-100-105.js）|
| 高考三等 106-114 全年度 | 2026-04-19 | 322→990 題，修 PUA 字元解碼 + x threshold |
| 藥師一階 104年 第二次 | 已存在（240 題） | c=306 s=11,22,44，早已正確爬入 |
| 藥師二階 105年 第一/二次 | 2026-04-18 | 補回 420 題（c=307 s=44,55,66） |
| 醫事檢驗師 105年 第一/二次 | 2026-04-18 | 補回 960 題（c=308） |
| 物理治療師 105年 第一/二次 | 2026-04-18 | 補回 960 題（c=311） |
| 醫事放射師 105年 第一/二次 | 2026-04-18 | 補回 959 題（c=309） |
| 社工師 112-114 第二次 | 2026-04-17 | c=103 session 112110/113100/114100 |

### ❌ 無法修復

| 項目 | 原因 |
|------|------|
| 職治師 102年 第二次 小兒疾病 | 6/80 題，PDF 格式特殊；MoEX 回 302 redirect（PDF 無法下載） |
| 中醫師(一) 100年 第二次 基礎(二) | 80/80 ✅ 用 OCR Vision 補齊（2026-04-19） |
| 醫師二階 103年 第一/二次 醫學(四) | 80/80 ✅ 用 OCR Vision 補齊（2026-04-19） |
| 中醫師(一) 109年 第一次 基礎(二) | 80/80 ✅ 用 OCR Vision 補齊（2026-04-19） |
| 中醫師(二) 109年 第一次 臨床(三)/(四) | 80/80 ✅ 用 OCR Vision 補齊（2026-04-19） |
| 護理師 106-109年 內外科/產兒/精神科 散落缺題 | MoEX 從未在現行電子系統上線這些科目的 PDF（c=101 s=0103/0104/0105 永久 302）；現有題目為舊系統存檔，缺題無法補 |
| 護理師 100/101年 部分散落缺題 | 同上 |

### ℹ️ 假性缺口（非真實缺口）

- 護理師/營養師各科：40/50 題 → 申論題正常排除
- 藥師二階法規 102-104年：50/80 題 → 該科本來只有 50 題
- 律師一試 105/106年 民法/民訴：74 題 → 62 單選 + 12 複選，非缺口
- 護理師 111年 基礎醫學、113年+ 全科：50 題/卷是新格式，非缺題
- ~~藥師二階 110年+ 藥物治療：54 題/卷是新格式（從 80 縮減），非缺題~~ **校正 2026-05-06**：實際 PDF 全為 80 題，原 scraper 在 Q54 切掉。已用 fill-small-gaps.js 補回 Q55-80。
