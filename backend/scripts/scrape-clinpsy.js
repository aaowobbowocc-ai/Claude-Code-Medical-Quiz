#!/usr/bin/env node
// 臨床心理師考試爬蟲。每卷格式為「一、申論題（跳過）+ 二、測驗題（單選，抓取）」。
...
const OUT = path.join(__dirname, '..', 'questions-clinical-psychology.json')
...
// 6 卷固定結構
const PAPERS = [
  { subject: '臨床心理學基礎', tag: 'cp_basic' },
  { subject: '臨床心理學總論(一)', tag: 'cp_general_1' },
  { subject: '臨床心理學總論(二)', tag: 'cp_general_2' },
  { subject: '臨床心理學特論(一)', tag: 'cp_special_1' },
  { subject: '臨床心理學特論(二)', tag: 'cp_special_2' },
  { subject: '臨床心理學特論(三)', tag: 'cp_special_3' },
]
// 場次：[code, c, s前2碼]
// 注意：106-110 年的 PDF 為無 ABCD 標記的雙欄格式（需 column parser + OCR 清理），
// 暫不納入；目前僅收 113/114 的標準標記格式。舊年份待後續補。
const SESSIONS = [
  ['113100', '111', '10'], ['114100', '111', '10'],
]
const sessionName = code => /0(2|3)0$/.test(code) ? '第一次' : '第二次'

async function getPdf(kind, code, c, s) {
  const fn = path.join(CACHE, `clinpsy_${kind}_${code}_c${c}_s${s}.pdf`)
...
async function main() {
  ...
  for (const [code, c, sb] of SESSIONS) {
    ...
    for (let pi = 0; pi < 6; pi++) {
      ...
      if (!qtext.includes('臨床心理師')) { console.log(`  ⚠ ${code} s${s} 類科不符，跳過`); continue }
