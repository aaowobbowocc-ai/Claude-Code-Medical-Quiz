...
// ─── HTTP helpers ──────────────────────────────────────────────────────────────
function fetchPdf(url, retries = 4) {
  return new Promise((resolve, reject) => {
    const req = https.get(url, {
      ...
      if (res.statusCode === 301 || res.statusCode === 302) {
        ...
        // Absolute redirect → follow it
        if (loc && loc.startsWith('http')) { res.resume(); return fetchPdf(loc, retries).then(resolve, reject) }
        // Relative redirect (error page) → permanent 302, fail fast.
        // (Retry logic removed: probe-alt-cs.js already validated working URLs.)
        res.resume()
      ...
      res.on('error', e => retries > 0
        ? setTimeout(() => fetchPdf(url, retries - 1).then(resolve, reject), 1000) : reject(e))
    })
